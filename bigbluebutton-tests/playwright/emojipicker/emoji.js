...
class Emoji extends Page {
  ...
  async characterLimit() {
    ...
    await this.page.fill(e.chatBox, e.frequentlyUsedEmoji.repeat(maxMessageLength));
    await this.waitAndClick(e.sendButton);
    await this.waitForSelector(e.chatUserMessageText);
    await expect(messageLocator).toHaveCount(1);

    await this.page.fill(e.chatBox, e.frequentlyUsedEmoji.repeat(maxMessageLength + 1));
